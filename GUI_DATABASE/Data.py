...
4. Display Books
5. Delete Books 
6. Enter Book ID Number: 
[1-4] to delete the record 
7. Edit Book enter Book ID
8. Record displays data
...
def Validate_Book_Copies():
    ...
    return passed_book_copies
# Create a update function
def update():
    conn = sqlite3.connect('Book.db')
# Create a variable called statement to update the variables 
    c = conn.cursor()
    book_record_id = delete_book.get()
    statment = """UPDATE books 
                  SET book_id = :book_id, 
                      book_name = :book_name, 
                      Author = :book_author, 
                      ISBN = :book_isbn, 
...
def edit():
    ...
    global delete_book_edit 
# Create Labels ands Text Boxes
    ...
    delete_book_edit = Entry(editor, width=30)
    delete_book_edit.grid(row=7, column=1)

    book_id_label = Label(editor, text="Book Qty")
    ...
    delete_book_label = Label(editor, text="Book ID")
...
# Connect DB
    conn = sqlite3.connect('Book.db')
    c = conn.cursor()
# Create for loop to iterate over the text box variables  
    book_record_id = delete_book.get()
    Statement = "SELECT * FROM books WHERE oid = " + book_record_id
    print(Statement)
    c.execute(Statement)
...
# Create a query to retrieve a values from db
def query():
    conn = sqlite3.connect('Book.db')
    c = conn.cursor()
...
# Create the variable text box and labels 
...
book_id_label = Label(root, text="Book Qty")
book_id_label.grid(row=0, column=0)
